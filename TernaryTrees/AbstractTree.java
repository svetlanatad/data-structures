...
public abstract class AbstractTree<E> implements Tree<E> {
  ...
  //(a) You didn't say implement efficiently, right? :)
  public ArrayList<Position<E>> ancestors(Position<E> p) {

    ArrayList<Position<E>> array = new ArrayList<>();
    while (!isRoot(p)) {
      p = parent(p);
      array.add(p);
    ...
    array.add(root());
    ArrayList<Position<E>> reversedArr = new ArrayList<>();
    for (int j = array.size() - 1; j >= 0; j--) {
      reversedArr.add(array.get(j));
    }
  ...
  public Iterable<Position<E>> path(Position<E> p1, Position<E> p2) {
    ArrayList<Position<E>> path = new ArrayList<>();
    ...
    if (p1 == p2) {
      ...
      ArrayList<Position<E>> path1 = ancestors(p1);
      ArrayList<Position<E>> path2 = ancestors(p2);
      //their common ancestor now has to be the root :)
      int i = 0;

      path.add(p1);
      for (int j = path1.size() - 1; j > i; j--) {
        path.add(path1.get(j));
      }
      for (int j = i + 1; j < path2.size(); j++) {
        if(!isRoot(path2.get(j))){
